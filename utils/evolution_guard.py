...
def write_journal_entry(
    event_type: str, payload: dict[str, Any], backup_dir: Path
) -> None:
    try:
...
def snapshot_skills_dir(skills_dir: Path, backup_dir: Path) -> Path | None:
    try:
        ...
        archive_path_str = shutil.make_archive(
            base_name=str(archive_name), format="zip", root_dir=str(skills_dir)
        )
...
def record_skill_deployment(
    skill_name: str, skill_path: Path, snapshot_path: Path | None, backup_dir: Path
) -> None:
...
def track_skill_error(
    skill_name: str, error_msg: str, skills_dir: Path, backup_dir: Path
) -> bool:
    """
    ...
    ERROR_TRACKER[skill_name] = [
        t for t in ERROR_TRACKER[skill_name] if t > one_hour_ago
    ]
    ...
    if len(ERROR_TRACKER[skill_name]) >= 3:
        logger.warning(
            f"Skill '{skill_name}' triggered {len(ERROR_TRACKER[skill_name])} errors in the last hour. Triggering auto-rollback."
        )
...
def rollback_to_latest_snapshot(
    skill_name: str, skills_dir: Path, backup_dir: Path
) -> bool:
    ...
    if journal_file.exists():
        try:
            ...
            for line in reversed(lines):
                ...
                if (
                    entry.get("event_type") == "skill_deployed"
    ...
    if not snapshot_path or not snapshot_path.exists():
        logger.info(
            "No specific snapshot found in journal for skill, looking for latest zip in backup directory."
        )
    ...
    if snapshot_path and snapshot_path.exists():
        ...
        write_journal_entry(
            "rollback_failed",
            {"skill_name": skill_name, "reason": "No valid snapshot found"},
            backup_dir,
        )
