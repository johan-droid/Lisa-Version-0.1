...
def load_api_keys(encrypted_file: str | Path, master_key: str | bytes) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError("Unable to decrypt API key vault with the supplied master key.") from exc
    return json.loads(decrypted.decode("utf-8"))


def save_api_keys(encrypted_file: str | Path, master_key: str | bytes, payload: dict[str, Any]) -> None:
    path = Path(encrypted_file).expanduser().resolve()
