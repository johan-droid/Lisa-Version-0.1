...
class SelfDirectedConductor:
    def __init__(self, settings: Settings, conductor: TaskConductor):
        ...
        self.recent_tasks = collections.deque(maxlen=10)
        self._generic_task_phrases = {
            "write unit tests",
            "update readme",
    ...
    async def start(self) -> None:
        if self._task is None:
            self._task = asyncio.create_task(
                self._loop(), name="self-directed-conductor"
            )
        ...
        while not self._stop.is_set():
            try:
                if self.conductor.is_idle():
                    LOGGER.info(
                        "System is idle. Generating autonomous candidate goals..."
                    )
...
"""
        try:
            llm_client = getattr(
                self.conductor.runtime,
                "llm_client",
                getattr(
                    getattr(self.conductor, "tool_executor", None), "llm_client", None
                ),
            )
            ...
            chat_resp = await llm_client.generate_brain(
                ...
                user_prompt=prompt,
                max_tokens=800,
                persona_weights={"distributed_mind": 1.0},
            )
            ...
            for attempt in [
                ...
                lambda: (
                    json.loads(response_text.split("```")[1].split("```")[0].strip())
                    if "```" in response_text
                    else None
                ),
                lambda: (
            ...
            if candidates is None:
                LOGGER.error(
                    "Failed to parse autonomous goals. Raw response:\n%s", response_text
                )
                return

            if not isinstance(candidates, list):
                LOGGER.warning(
                    "LLM response was not a JSON list. Raw response:\n%s", response_text
                )
                return
            ...
            for cand in candidates:
                name = (
                    cand.get("Task name") or cand.get("task_name") or cand.get("name")
                )
                if not name:
                    continue
                name = str(name).strip()
                if not self._is_candidate_actionable(name):
                    LOGGER.info("Skipping low-signal autonomous candidate: %s", name)
                    continue
                value = float(
                    cand.get("Estimated value")
                    or cand.get("estimated_value")
                    or cand.get("value")
                    or 1.0
                )
                urgency = float(
                    cand.get("Estimated urgency")
                    or cand.get("estimated_urgency")
                    or cand.get("urgency")
                    or 1.0
                )
                risk = float(
                    cand.get("Estimated risk")
                    or cand.get("estimated_risk")
                    or cand.get("risk")
                    or 1.0
                )
                cost = float(
                    cand.get("Estimated cost")
                    or cand.get("estimated_cost")
                    or cand.get("cost")
                    or 1.0
                )
            ...
            if best_candidate and best_score >= threshold:
                task_name = (
                    best_candidate.get("Task name")
                    or best_candidate.get("task_name")
                    or best_candidate.get("name")
                )

                if task_name in self.recent_tasks:
                    LOGGER.info("Skipping duplicate autonomous task: %s", task_name)
                else:
                    self.recent_tasks.append(task_name)
                    LOGGER.info(
                        "Selected autonomous task: %s with score %.2f",
                        task_name,
                        best_score,
                    )
    ...
    async def _get_recent_notepad_activity(self) -> str:
        try:
            entries = await asyncio.to_thread(
                self.conductor.runtime.notepad.latest_entries, 5
            )
            if not entries:
            ...
            for entry in entries:
                summary.append(
                    f"[{entry.get('entry_type')}] {str(entry.get('payload'))[:100]}"
                )
            return "\n".join(summary)
        except Exception:
            return "unknown"

    def _is_candidate_actionable(self, name: str) -> bool:
        normalized = " ".join(name.lower().split())
        if len(normalized) < 12:
            return False
        if normalized in self._generic_task_phrases:
            return False
        blocked_fragments = ("maybe", "consider", "brainstorm", "explore", "something")
