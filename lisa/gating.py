...
class PersonaGatingNetwork:
    ...
    def save(self, path: Path) -> None:
        ...
        master_key = os.environ.get("LISA_MASTER_KEY") or os.environ.get("LISA_KEYS_MASTER_KEY", "default-insecure-key-do-not-use-in-prod")
        sig = hmac.new(master_key.encode("utf-8"), json_data, hashlib.sha256).hexdigest()
        ...
        json_path.write_text(json.dumps(signed_metadata, indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    def load(cls, path: Path) -> "PersonaGatingNetwork":
        ...
        if path.exists() and path.suffix == ".pkl":
            ...
            elif isinstance(state, dict) and "vectorizer" in state and "classifier" in state:
                network = cls.initialize()
        ...
        if not npz_path.exists() or not json_path.exists():
            raise FileNotFoundError(f"Gating model files not found: {npz_path}, {json_path}")
        ...
        master_key = os.environ.get("LISA_MASTER_KEY") or os.environ.get("LISA_KEYS_MASTER_KEY", "default-insecure-key-do-not-use-in-prod")
        expected_sig = hmac.new(master_key.encode("utf-8"), json.dumps(metadata, ensure_ascii=False).encode("utf-8"), hashlib.sha256).hexdigest()

        if not hmac.compare_digest(signature, expected_sig):
            raise PermissionError("Gating model signature verification failed. The file may have been tampered with.")
        ...
        if "idf_" in state["encoder"] and isinstance(state["encoder"]["idf_"], list):
            state["encoder"]["idf_"] = np.array(state["encoder"]["idf_"], dtype=np.float32)
    ...
    def load_or_initialize(cls, path: Path) -> "PersonaGatingNetwork":
        ...
        if npz_path.exists() or path.exists():
            try:
                ...
                import logging
                logging.getLogger("lisa.gating").warning(f"Failed to load gating model: {e}. Reinitializing.")
        network = cls.initialize()
